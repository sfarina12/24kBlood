...
public class shooting : MonoBehaviour
{
    ...
    public bool isPrimaryFire = true;
    [Space]
    ...
    float timer_timeBetweenShots = 0;
    void Start() { 
        if(input == null) { 
    ...
    void Update() {
        ...
        if(isPrimaryFire) { isShooting = input.inputSet.Combat.Shoot.ReadValue<float>() == 1 ? true : false; }
        else { isShooting = input.inputSet.Combat.Throw.ReadValue<float>() == 1 ? true : false; }

        bool isInteracting = itemAnimator.GetInteger("isInteracting") == 1 ? true : false;
        
        if(isShooting && !isInteracting) {
            if(timer_timeBetweenShots <= 0) {
                int value = Random.Range(1, numberOfAnimations);
                itemAnimator.SetInteger(choiseVariableName, value);

                timer_timeBetweenShots = timeBetweenShots;
                ...
                //     }
                // }
            } else { timer_timeBetweenShots -= Time.deltaTime; }
        }
    }
}
